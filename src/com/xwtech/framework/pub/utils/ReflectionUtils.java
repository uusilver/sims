...
public final class ReflectionUtils extends org.springframework.util.ReflectionUtils
{
	 ...
	 * @param propertyName 提供类型信息的Class的属性.
	 */
	public static Object convertHibernateValue(Object value, Class<?> clazz, String propertyName)
	{
		try
		{
			Class<?> toType = null;
			...
			{
				String refBeanName = propertyName.split("\\.")[0];
				String refBeanPropertyName = propertyName.split("\\.")[1];
				Class<?> refType = BeanUtils.getPropertyDescriptor(clazz, refBeanName).getPropertyType();
				toType = BeanUtils.getPropertyDescriptor(refType, refBeanPropertyName).getPropertyType();
			} else
				toType = BeanUtils.getPropertyDescriptor(clazz, propertyName).getPropertyType();
			//将集合类型中的每个元素转换为与查询字段相匹配类型
			if (value instanceof Collection)
			{
				...
				if (((Collection) value).size() > 0)
				{
					for (Object item : (Collection) value)
					{
						...
						dc.setPatterns(new String[] { "yyyy-MM-dd", "yyyy-MM-dd HH:mm:ss" });
						ConvertUtils.register(dc, Date.class);
						collection.add(ConvertUtils.convert(item, toType));
					}
				}
				return collection;
			}
